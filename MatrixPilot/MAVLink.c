 ...
 */
	if (target_system != mavlink_system.sysid)
	{
		DPRINT("invalid system id %u expected %u\r\n", target_system, mavlink_system.sysid);
		return true;
	}
	if (target_component != mavlink_system.compid)
	{
		DPRINT("invalid component id %u expected %u\r\n", target_component, mavlink_system.compid);
//		return true;
	}
	return false;
}
...
static void MAVLinkDataStream(mavlink_message_t* handle_msg) // MAVLINK_MSG_ID_DATA_STREAM
{
	...
	mavlink_msg_data_stream_decode(handle_msg, &packet);
...
//} mavlink_data_stream_t;
	...
	DPRINT("MAVLINK_MSG_ID_DATA_STREAM %u\r\n", handle_msg->msgid);
}
...
//	DPRINT("handleMessage(sysid %u, compid %u, msgid %u)\r\n", handle_msg->sysid, handle_msg->compid, handle_msg->msgid);
	...
	{
		DPRINT("CURIOUS: handling_of_message_completed already TRUE!\r\n");
		return;
	}
	...
	if (handling_of_message_completed == true)
	{
//		DPRINT("MAV MSG 0x%x handled by MAVParamsHandleMessage\r\n", handle_msg->msgid);
		return;
	}
	handling_of_message_completed |= MAVMissionHandleMessage(handle_msg);
	if (handling_of_message_completed == true)
	{
//		DPRINT("MAV MSG 0x%x handled by MAVMissionHandleMessage\r\n", handle_msg->msgid);
		return;
	}
	handling_of_message_completed |= MAVFlexiFunctionsHandleMessage(handle_msg);
	if (handling_of_message_completed == true)
	{
//		DPRINT("MAV MSG 0x%x handled by MAVFlexiFunctionsHandleMessage\r\n", handle_msg->msgid);
		return;
	}
...
enum MAV_STATE
{
	...
	MAV_STATE_ENUM_END=8, /*  | */
		...
		if (dcm_flags._.calib_finished == 1)
			mavlink_system_status = MAV_STATE_STANDBY;
		else if (dcm_flags._.init_finished == 1)
			mavlink_system_status = MAV_STATE_ACTIVE;
		else
...
int32_t alt_imu = IMUlocationz._.W1;
int32_t barometric_alt = get_barometer_altitude();
int32_t range_finder_alt = 34;

		mavlink_msg_altitudes_send(MAVLINK_COMM_0, msec, alt_sl_gps.WW, alt_imu, barometric_alt, 0, range_finder_alt, 0);
...
//inline long get_barometer_pressure(void)     { return barometer_pressure; }
		mavlink_msg_raw_pressure_send(MAVLINK_COMM_0, msec, get_barometer_pressure() / 10, 0, 0, get_barometer_temperature());
		//mavlink_msg_raw_pressure_send(mavlink_channel_t chan, uint64_t time_usec, int16_t press_abs, int16_t press_diff1, int16_t press_diff2, int16_t temperature)
#endif
	}
...
float nav_pitch = 0.0;
//		mavlink_heading = get_geo_heading_angle() * 100;    // mavlink global position expects heading value x 100
int16_t nav_bearing = get_geo_heading_angle();
int16_t target_bearing = (desired_dir < 0) ? (desired_dir + 360) : desired_dir;
uint16_t wp_dist = tofinish_line;
float alt_error = 0.0;
